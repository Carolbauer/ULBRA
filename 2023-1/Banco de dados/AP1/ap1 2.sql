...
insert into clientes(nome, cnpj,endereco,telefone)
values('construsul','61626097000137', 'Arroio do Sal', '5199999999'),
	('construfacil','42683776000101', 'Rondinha', '51888888888'),
    ('construforte','87922524000118', 'Perola', '51777777777'),
    ('PedrãoMC', '22353455000123', 'Arroio do sal', '51555555555');
...
CREATE VIEW caminhoes_por_funcionario2 AS
    ...
    /*count conta quantas vezes o id_maq foi carregado por um funcionario naquele dia*/
        FROM funcionarios f
            inner join maquinas m on f.id = m.id_func # junta tabela maquinas com tabela funcionarios
            ...
            SELECT nome_funcionario, quantidade_caminhoes_carregados
                FROM caminhoes_por_funcionario2
                order by quantidade_caminhoes_carregados desc limit 1;
...
CREATE VIEW produtos_mais_vendidos_por_cliente AS
    SELECT c.nome AS nome_cliente, p.descricao AS descricao_produto, sum(v.qnt_carga) AS produtos_mais_vendidos , sum(p.preco_m3*v.qnt_carga) AS valor_total_vendas#somando a quantidade de carga de produtos por cliente
        FROM produtos p
            ...
            group by nome_cliente, valor_total_vendas #pesquisa agrupando pelo nome do cliente e descricao do produto
...
/*procedure que consulta o ultimo id gerado, se o cnpj já existe e  insere novo cliente*/
DELIMITER $$
...
BEGIN
    ...
    #verificando se o CNPJ já existe na tabela de clientes
    IF EXISTS (SELECT * FROM clientes WHERE cnpj = c_cnpj) THEN
        SELECT 'Cliente já existe' AS mensagem;
    ELSE
        #inserindo novo registro na tabela clientes
        INSERT INTO clientes(id,nome,cnpj,endereco,telefone)
            values (c_id,nome,c_cnpj,endereco,telefone);
        SELECT 'Cliente inserido com sucesso' AS mensagem;
    END IF;
END $$
DELIMITER ;
call inserir_clientes('materiais rocha','87678043671238','praia real', '88888888'); #inserindo novo cliente
